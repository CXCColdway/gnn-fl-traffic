...
def load_cifar10(args):
    data_dir = "./data/" + str(args.dataset)  # 数据集存储地址
    ...
    train_transforms = [Crop(32, 32), FlipLR(), Cutout(8, 8)]  # 数据扩增操作

    # 划分数据集
    train_user_groups, test_user_groups, A = split_equal_noniid(
        train_set, test_set, args.shards, args.edge_frac, args.clients)

    train_set = list(zip(transpose(normalise(pad(train_set.data, 4), data_mean, data_std)), train_set.targets))# 获取训练集
    test_set = list(zip(transpose(normalise(test_set.data, data_mean, data_std)), test_set.targets))# 获取测试集

    # 将训练集和测试集转换为各个用户客户端上用于训练的小批量数据
    train_batches = []
    test_batches = []
...
def split_equal_noniid(train_dataset, test_dataset, shards, edge_frac, clients):
    ...
    idxs = idxs_labels[0, :]
    label_count = np.bincount(idxs_labels[1])  # 统计标签数组中各数字（从小到大顺序）出现的个数
    ...
    for i in range(clients):
        ...
        sample_size = [np.count_nonzero(selected_labels == j) for j in label_type]   # 统计列表中非零元素的个数作为采样的大小
        int(shard_size * shards / len(label_type))
        dict_label_dist[i] = np.array((label_type, sample_size))

        for j, l in enumerate(label_type):  # 遍历label_type的索引值和元素值
...
def normalise(x, mean, std):    # 正态化函数
...
def pad(x, border=4):  # 矩阵填充函数
    return np.pad(x, [(0, 0), (border, border), (border, border), (0, 0)], mode='reflect')


def transpose(x, source='NHWC', target='NCHW'):   # 矩阵转置

...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x, args):
        return torch.relu(self.conv(x)).to(args.device)
...
class SpatioConvLayer(nn.Module):
    ...
    def forward(self, x, args):
        x = x.transpose(0, 3)
        x = x.transpose(1, 3)
        g = self.g.to(args.device)
        output = self.gc(g, x).to(args.device)
        output = output.transpose(1, 3).to(args.device)
        output = output.transpose(0, 3).to(args.device)
...
class FullyConvLayer(nn.Module):
    ...
    def forward(self, x, args):
        return self.conv(x).to(args.device)
...
class OutputLayer(nn.Module):
    ...
    def forward(self, x, args):
        ...
        return self.fc(x_t2, args).to(args.device)
...
class STGCN_WAVE(nn.Module):
    ...
    def forward(self, x, args):# x.size[50, 1, 144, 207]
        for i in range(self.num_layers):
            i_layer = self.control_str[i]
            if i_layer == "N":
                x = self.layers[i](x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
                '''
                RuntimeError: Given normalized_shape=[207, 16], expected input with shape [*, 207, 16], but got input of size[50, 16, 143, 207]
                '''
            else:
                x = self.layers[i](x, args)
        return self.output(x, args)

    def get_state(self):
        return self.state_dict(), []

    def set_state(self, w_server, w_local):
        self.load_state_dict(w_server, w_local)
